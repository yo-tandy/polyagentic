...
class JerryAgent(Agent):
    ...
    Session-based (stateful). Handles `assign_ticket` actions by creating
    tasks on the board and sending assignment messages to agents.
    """
    ...
    async def _parse_response(self, result_text: str, original_msg: Message) -> list[Message]:
        ...
        for action in actions:
            ...
            if action_type == "assign_ticket":
                # Creates a task on the board AND sends message to the assignee
                to = action.get("to", "")
